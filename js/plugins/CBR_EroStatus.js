...
CBR["エロステータス"] = function (ary) {
  ...
  CBR.eroStatus.data[CBR.eroStatus.addPage] = CBR.eroStatus.data[
    CBR.eroStatus.addPage
  ] || { 画像: [], テキスト: [] }; //ページがなかったら作る
  CBR.eroStatus.data[CBR.eroStatus.addPage][obj.type][
    CBR.eroStatus.data[CBR.eroStatus.addPage][obj.type].length
  ] = obj; //んで入れる
};
...
Window_EroStatus.prototype.update = function () {
  ...
  if (
    CBR.eroStatus.data.length !== 1 &&
    ImageManager.isReady() &&
    FontManager.isReady()
  ) {
    //読み込み終わってたら、結局画像読み終えてない限りリフレッシュする必要なし
    ...
    if (n !== undefined && CBR.eroStatus.pageNow != n) {
      //ページの変更だったら
      CBR.eroStatus.pageNow = n;
      AudioManager.playSe({
        name: CBR.eroStatus.se,
        volume: CBR.eroStatus.seVol,
        pitch: 100,
        pan: 0,
      });
      this._CBR_drawn = false;
      this.refresh();
    } else if (!this._CBR_drawn) {
      //画像が読み込まれてるというのに描写されて無い時
      this.refresh();
    }
  }
};
...
Window_EroStatus.prototype.drawText = function (
  text,
  x,
  y,
  maxWidth,
